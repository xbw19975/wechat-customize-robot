...
import { getMeizi, getCoser, getTianGouRiJi, getWRYl, getHistory } from './axios.js';
...
export class ChatGPTBot {
  ...
  async onMessage(message: Message) {
    ...
    if (this.triggerGPTMessage(rawText, privateChat)) {
      ...
      console.log("triggerGPTMessage", text);

      // 使用DallE生成图片
      if (text.indexOf("看美女") != -1 || text.indexOf("看coser") != -1) {
        console.log(`🤖 Image: ${text}`)
        let url = 'https://p1.toutiaoimg.com/origin/1372d000231d004f74095';
        let name = 'meinv.jpg';
        if (text.indexOf("看美女") != -1) {
          url = await getMeizi() as string;
          name = 'meinv.jpg'
        } else if (text.indexOf("看coser") != -1) {
          url = await getCoser() as string;
          name = 'coser.jpg'
        }
        console.log('Image Url', url);

        if (privateChat) {
          const fileBox = FileBox.fromUrl(url, { name })
          message.say(fileBox)
        } else {
          const fileBox = FileBox.fromUrl(url, { name })
          message.say(fileBox)
        ...
        return;
      } else if (text.indexOf("舔狗日记") != -1) {
        let textR = await getTianGouRiJi() as string;
        message.say(textR)
        return;
      } else if (text.indexOf("温柔语录") != -1) {
        let textR = await getWRYl() as string;
        message.say(textR)
        return;
      } else if (text.indexOf("历史上的今天") != -1) {
        let textR = await getHistory() as string;
        message.say(textR)
        return;
      }
      if (privateChat) {
